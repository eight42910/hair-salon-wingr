...
export const NoticeCard: React.FC<NoticeCardProps> = ({
  ...
  if (variant === 'compact') {
    return (
      <Link
        ...
        className={`group block w-full px-4 py-5 border-b border-border transition-colors duration-200 hover:bg-surface2 ${className}`}
      >
  ...
  const content = 'content' in notice ? notice.content : '';
  const excerpt = stripHtml(content).slice(0, 120);
  const thumbnailImage =
    'thumbnailImage' in notice ? notice.thumbnailImage : undefined;
  const cardVariants = {
    default: 'p-6',
    featured: 'p-8',
